...
PATH = path_dic.get(con.WORDPIECE)
NUM_CLASSES = 3
NCOLS = 768
NUM_EPOCHS = 10
NUM_FILTERS = 1024
...
class RobertaClass(torch.nn.Module):
    def __init__(self, max_rlen, kernel_size, class_num):
...
def valid(model, testing_loader, loss_function):
    ...
    with open(PATH + "RoBERTa_CNN_scores_" + str(1) + ".txt", 'w') as outfile:
...
def main():
    ...
    model = RobertaClass(MAX_LEN, KERNEL_SIZE, NUM_CLASSES)
    model.to(device)
